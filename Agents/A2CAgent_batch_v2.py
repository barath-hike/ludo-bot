...
class Agent:
    ...
    def act(self, s, action_list):
        ...
        state = np.reshape(s, [1, self.state_size])
    ...
    def learn(self, state, action, reward, state_, done):
        ...
        state[:,0] = state[:,0]/6
        state[:,1:9] = state[:,1:9]/69
        state[:,9:17] = state[:,9:17]/138

        state_ = np.array(state_).astype('float32')
        state_[:,0] = state_[:,0]/6
        state_[:,1:9] = state_[:,1:9]/69
        state_[:,9:17] = state_[:,9:17]/138

        state = np.reshape(state, [state.shape[0], self.state_size])
        state_ = np.reshape(state_, [state.shape[0], self.state_size])
        state = tf.convert_to_tensor(state, dtype=tf.float32)
    ...
    def load_actor(self, name):
        self.actor.built = True
        self.actor.model.load_weights(name)
