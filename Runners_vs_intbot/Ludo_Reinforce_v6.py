...
def run_game(num_ep, model_output):
    ...
    agent0_reward = []
    agent1_reward = []
    output_dir = 'model_output_vs_intbot/REINFORCE_v6/%s/' % model_output
    ...
    for ep in tqdm(range(0, num_ep), ascii=True, unit="e"):
        ...
        for it in range(0, batch_size):
            traj, episode_reward = generate_episode(agent0, env, bot0)
...
def generate_episode(agent, env, bot0):
        ...
        if player_turn == player_turn_temp:
            ...
            if action_list:
                if player_turn == 0:
                    ...
                    s_ = env.return_state()
                    action = bot0.act(state=s_, p=player_turn)
