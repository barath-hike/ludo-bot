...
class FullBoard:
    ...
    def make_step(self, a):
        ...
        temp_risk1 = np.array(risk1)*np.array(self.state_reward[:4])
        risk1 = np.sum(np.array(risk1)*np.array(self.state_reward[:4]))

        risk2 = self.risk(self.convert_state(1))
        temp_risk2 = np.array(risk2)*np.array(self.state_reward[4:])
        ...
        return self.state, self.reward, (game_over != -1), self.player_turn, temp_risk1, temp_risk2
    ...
    def risk(self, s):
        ...
        for i in range(self.pieces):
            ...
            for j in range(self.pieces):
                mov = s[1 + i] - s[1 + self.pieces + j]
                if mov >= 1 and mov<=6:
                    p += 1/6
                elif mov >= 7 and mov <= 12:
                    p += 1/36
                elif mov >= 13 and mov <= 17:
                    p+= 1/(6*6*6)
            probs.append(p)
        return probs
