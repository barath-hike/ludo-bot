...
class FullBoard:
    ...
    def make_step(self, a):
        ...
        self.reward = [(np.sum(self.state_reward[:4]) - np.sum(old_state_reward[:4])) - (np.sum(self.state_reward[4:8]) - np.sum(old_state_reward[4:8])),
                       (np.sum(self.state_reward[4:8]) - np.sum(old_state_reward[4:8])) - (np.sum(self.state_reward[:4]) - np.sum(old_state_reward[:4]))]

        if self.cut_list[self.action_list.index(a)] == 1:
            self.six_flag = 0
            temp_reward = self.exp_reward(self.state, self.state_reward, self.player_turn)
            self.reward[self.player_turn] += temp_reward
        ...
        for j in range(0, self.pieces):
            ...
            if old_pos != self.end[p]:
                ...
                if new_pos <= self.end[p]:
                    ...
                    if is_pos_occupied != 0 and self.state[is_pos_occupied] not in self.safe:  # move knocks off opponent
    ...
    def exp_reward(self, state, state_reward, p):
        reward = 0
        for d in range (1, 7):
            reward += self.find_possible_moves_(state, state_reward, p, d)/6
        return reward

    def find_possible_moves_(self, state, state_reward, p, d):
        ...
        for j in range(0, self.pieces):
            ...
            temp_state = np.copy(state).tolist()
            temp_state[0] = d
            temp_state_reward = np.copy(state_reward).tolist()

            old_pos = temp_state[1 + p * self.pieces + j]
            if old_pos != self.end[p]:
                new_pos = (old_pos + temp_state[0])
                ...
                if new_pos <= self.end[p]:
                    temp_state[1 + p * self.pieces + j] = new_pos
                    temp_state_reward[p * self.pieces + j] += temp_state[0]
                    ...
                    state_reward_list.append(temp_state_reward)
                    action_list.append(j)

        length = len(action_list)
        exp_reward = 0
        for i, a in enumerate(action_list):
            temp = np.sum(state_reward_list[i][self.pieces * p : self.pieces * (p + 1)]) - np.sum(state_reward[self.pieces * p : self.pieces * (p + 1)])
            exp_reward += temp / length
