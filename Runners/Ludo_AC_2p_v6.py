...
def run_game(num_ep, model_output):
    ...
    output_dir_a = 'model_output/A2C_v6/%s/actor/' % model_output
    output_dir_c = 'model_output/A2C_v6/%s/critic/' % model_output
    ...
    for ep in tqdm(range(0, num_ep), ascii=True, unit="e"):
        ...
        while not game_over:
            ...
            if game_over:
                print(np.sum(episode_reward[0]), np.sum(episode_reward[1]))
